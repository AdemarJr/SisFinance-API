...
function getAdminClient() {
  if (!isSupabaseConfigured() || !isServiceRoleConfigured()) {
    throw new Error(
      "SUPABASE_SERVICE_ROLE_KEY n\xE3o configurada em server/.env \u2014 veja Supabase Dashboard \u2192 Settings \u2192 API"
    );
  }
  if (!adminClient) {
...
function getAnonClient() {
  if (!isSupabaseConfigured() || !isAnonConfigured()) {
    throw new Error("SUPABASE_ANON_KEY n\xE3o configurada em server/.env");
  }
  if (!anonClient) {
...
var secret = () => encoder2.encode(config.jwtSecret);
...
async function loadClienteSistema(authUserId) {
  if (!isServiceRoleConfigured()) return null;
  const supabase = getAdminClient();
  const { data, error } = await supabase.from("clientes_sistema").select("*, plano:planos_assinatura(*)").eq("auth_user_id", authUserId).maybeSingle();
...
authRoutes.post("/login", async (c) => {
  try {
    ...
    if (error || !data.user) {
      return c.json({ error: error?.message || "Credenciais inv\xE1lidas" }, 401);
    }
...
authRoutes.get("/me", async (c) => {
  ...
  if (!payload) return c.json({ error: "Token inv\xE1lido" }, 401);
...
authRoutes.post("/admin/users", async (c) => {
  ...
  try {
    ...
    if (authError || !authData.user) {
      return c.json({ error: authError?.message || "Erro ao criar usu\xE1rio" }, 400);
    }
    ...
    return c.json({ error: error instanceof Error ? error.message : "Erro ao criar usu\xE1rio" }, 500);
  }
});
...
async function executeDbQuery(payload) {
  const supabase = getAdminClient();
  ...
  try {
    ...
    if (payload.type === "insert") {
      const result = await supabase.from(payload.table).insert(payload.data);
      return { data: result.data, error: result.error };
    }
    ...
    return { data: null, error: { message: "Opera\xE7\xE3o inv\xE1lida" } };
  } catch (error) {
    return {
      data: null,
...
async function executeRpc(payload) {
  const supabase = getAdminClient();
...
var getByPrefix = async (prefix) => {
  const supabase = getAdminClient();
  const { data, error } = await supabase.from("kv_store_b1600651").select("value").like("key", `${prefix}%`);
  if (error) throw new Error(error.message);
...
async function initializeDatabase() {
  const supabase = getAdminClient();
  try {
    const { data: planosExistentes, error: checkPlanosError } = await supabase.from("planos_assinatura").select("id").limit(1);
      ...
      await supabase.from("planos_assinatura").insert([
        {
          ...
          limite_empresas: 1,
          preco_mensal: 0,
          descricao: "1 empresa, recursos b\xE1sicos",
          recursos: ["Dashboard b\xE1sico", "1 empresa", "Suporte por email"]
        },
        {
          nome: "Iniciante",
          limite_empresas: 3,
          preco_mensal: 97,
          descricao: "At\xE9 3 empresas, recursos intermedi\xE1rios",
          recursos: ["Dashboard completo", "At\xE9 3 empresas", "Relat\xF3rios b\xE1sicos", "Suporte priorit\xE1rio"]
        },
        {
          nome: "Profissional",
          limite_empresas: 10,
          preco_mensal: 297,
          descricao: "At\xE9 10 empresas, recursos avan\xE7ados",
          recursos: ["Dashboard avan\xE7ado", "At\xE9 10 empresas", "Relat\xF3rios avan\xE7ados", "API Access", "Suporte 24/7"]
        },
        {
          nome: "Enterprise",
    ...
    if (checkClientesError) {
      return {
        success: false,
        message: "Tabelas do sistema n\xE3o encontradas. Execute o SQL manualmente no Supabase Dashboard.",
        sql_file: "/supabase-auth-saas-setup.sql"
      };
    }
    const { data: adminExistente } = await supabase.from("clientes_sistema").select("*").eq("email", "admin@sisfinance.com").maybeSingle();
    return {
      success: true,
...
async function checkDatabaseStatus() {
  ...
  try {
    ...
    if (status.clientes_sistema) {
      const { data: adminData } = await supabase.from("clientes_sistema").select("id").eq("email", "admin@sisfinance.com").maybeSingle();
...
legacyRoutes.get("/db-status", async (c) => {
  try {
    ...
    return c.json({
      success: true,
      status,
      message: status.clientes_sistema ? "Banco de dados configurado corretamente" : "Tabelas do sistema n\xE3o encontradas. Execute /init-db ou o SQL manualmente."
    });
  } catch (error) {
...
legacyRoutes.get("/produtos/:empresa_id", async (c) => {
  try {
    ...
    const produtos = await getByPrefix(`produto:${empresaId}:`);
    return c.json({
      success: true,
      data: produtos,
  ...
  try {
    ...
    if (!produto.empresa_id || !produto.codigo || !produto.nome) {
      return c.json(
        {
          success: false,
          error: "Campos obrigat\xF3rios: empresa_id, codigo, nome"
        },
        400
      );
    }
...
legacyRoutes.put("/produtos/:empresa_id/:produto_id", async (c) => {
  try {
    ...
    const key = `produto:${empresaId}:${produtoId}`;
    const produtoExistente = await get2(key);
    if (!produtoExistente) {
      return c.json(
        {
          success: false,
    ...
    const produtoAtualizado = {
      ...
      ...dadosAtualizacao,
      id: produtoId,
      // Garante que o ID não mude
      empresa_id: empresaId
      // Garante que a empresa não mude
    };
    await set(key, produtoAtualizado);
    return c.json({
      success: true,
  ...
  try {
    ...
    if (!mov.empresa_id || !mov.produto_id || !mov.tipo_movimentacao || !mov.quantidade) {
      return c.json(
        {
          success: false,
          error: "Campos obrigat\xF3rios: empresa_id, produto_id, tipo_movimentacao, quantidade"
        },
        400
      );
    }
    const tiposValidos = ["Entrada", "Sa\xEDda", "Ajuste", "Perda"];
    if (!tiposValidos.includes(mov.tipo_movimentacao)) {
      return c.json(
        {
          success: false,
          error: `Tipo de movimenta\xE7\xE3o inv\xE1lido. Use: ${tiposValidos.join(", ")}`
        },
        400
      );
    }
    ...
    const produto = await get2(prodKey);
    if (!produto) {
      return c.json(
        {
          success: false,
    ...
    } else if (mov.tipo_movimentacao === "Sa\xEDda" || mov.tipo_movimentacao === "Perda") {
      if (produto.estoque_atual < mov.quantidade) {
        return c.json(
          {
            success: false,
            error: `Estoque insuficiente. Dispon\xEDvel: ${produto.estoque_atual}`
          },
          400
        );
      }
    ...
    return c.json({
      ...
      message: "Movimenta\xE7\xE3o registrada com sucesso"
    });
  } catch (error) {
    console.error("Erro ao registrar movimenta\xE7\xE3o:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
legacyRoutes.get("/movimentacoes/:empresa_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const movimentacoes = await getByPrefix(`movimentacao:${empresaId}:`);
    ...
    console.error("Erro ao listar movimenta\xE7\xF5es:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
legacyRoutes.get("/movimentacoes/:empresa_id/:produto_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const produtoId = c.req.param("produto_id");
    const todasMovimentacoes = await getByPrefix(`movimentacao:${empresaId}:`);
    const movimentacoesProduto = todasMovimentacoes.filter((m) => m.produto_id === produtoId);
    ...
    console.error("Erro ao listar movimenta\xE7\xF5es do produto:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
legacyRoutes.get("/projecao/:empresa_id", async (c) => {
  try {
    ...
    const data30DiasAtras = /* @__PURE__ */ new Date();
    ...
    todasMovimentacoes.forEach((mov) => {
      ...
      if (dataMov >= data30DiasAtras) {
        if (mov.tipo_movimentacao === "Sa\xEDda" || mov.tipo_movimentacao === "Perda") {
    ...
    Object.keys(consumoPorProduto).forEach((produtoId) => {
      ...
      consumoPorProduto[produtoId] = soma / 30;
    });
    const projecoes = produtos.filter((p) => p.estoque_atual < p.estoque_minimo && p.ativo).map((p) => {
      const consumoMedioDiario = consumoPorProduto[p.id] || 2;
      const diasProjecao = 30;
      const quantidadeNecessaria = p.estoque_minimo - p.estoque_atual + consumoMedioDiario * diasProjecao;
      return {
        produto_id: p.id,
        produto_nome: p.nome,
    ...
    console.error("Erro ao calcular proje\xE7\xE3o:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
legacyRoutes.get("/estoque/stats/:empresa_id", async (c) => {
  try {
    ...
    const stats = {
      ...
      produtos_abaixo_minimo: produtosAtivos.filter((p) => p.estoque_atual < p.estoque_minimo).length,
      produtos_zerados: produtosAtivos.filter((p) => p.estoque_atual === 0).length,
      produtos_ok: produtosAtivos.filter((p) => p.estoque_atual >= p.estoque_minimo).length
    };
    return c.json({
      success: true,
      data: stats
    ...
    console.error("Erro ao calcular estat\xEDsticas:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
...
legacyRoutes.get("/categorias-produtos/:empresa_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const categorias = await getByPrefix(`categoria_produto:${empresaId}:`);
    return c.json({
      success: true,
      data: categorias,
  ...
  try {
    ...
    if (!categoria.empresa_id || !categoria.nome) {
      return c.json(
        {
          success: false,
          error: "Campos obrigat\xF3rios: empresa_id, nome"
        },
        400
      );
    }
...
legacyRoutes.post("/pyroustock/config", async (c) => {
  try {
    ...
    if (!config2.empresa_id || !config2.api_key || !config2.project_id) {
      return c.json(
        {
          success: false,
          error: "Campos obrigat\xF3rios: empresa_id, api_key, project_id"
        },
        400
      );
    }
    const key = `pyroustock_config:${config2.empresa_id}`;
    await set(key, {
      empresa_id: config2.empresa_id,
      api_key: config2.api_key,
      project_id: config2.project_id,
      company_id: config2.company_id || null,
      nome_integracao: config2.nome_integracao || "PyrouStock",
      sincronizacao_ativa: config2.sincronizacao_ativa ?? true,
    ...
    console.error("Erro ao salvar configura\xE7\xE3o PyrouStock:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : String(error)
  ...
  try {
    ...
    if (!empresa_id) {
      return c.json(
        {
          success: false,
          error: "Campo obrigat\xF3rio: empresa_id"
        },
        400
      );
    }
    const configKey = `pyroustock_config:${empresa_id}`;
    const config2 = await get2(configKey);
    if (!config2) {
      return c.json(
        {
          success: false,
          error: "Configura\xE7\xE3o PyrouStock n\xE3o encontrada. Configure primeiro."
        },
        404
      );
    }
    const baseUrl = `https://${config2.project_id}.supabase.co/functions/v1/make-server-8a20b27d/integration`;
    const params = new URLSearchParams();
    if (config2.company_id) params.append("companyId", config2.company_id);
    ...
    const response = await fetch(url, {
      headers: {
        Authorization: `Bearer ${config2.api_key}`
      }
    });
    if (!response.ok) {
      ...
      for (const prodPyrou of pyrouData.inventory) {
        try {
          ...
          await set(produtoKey, {
            id: prodPyrou.id,
            empresa_id,
            codigo: prodPyrou.id.slice(-8),
            // Usar últimos 8 chars como código
            nome: prodPyrou.name,
    ...
    config2.ultima_sincronizacao = (/* @__PURE__ */ new Date()).toISOString();
    await set(configKey, config2);
    return c.json({
      success: true,
      data: {
        ...resultado,
      ...
      message: `Sincroniza\xE7\xE3o conclu\xEDda: ${resultado.produtos_sincronizados} produtos, ${resultado.vendas_sincronizadas} vendas, ${resultado.caixas_sincronizados} caixas`
    });
  } catch (error) {
...
legacyRoutes.get("/pyroustock/produtos/:empresa_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const produtos = await getByPrefix(`produto_pyrou:${empresaId}:`);
    return c.json({
      success: true,
      data: produtos,
      total: produtos.length
...
legacyRoutes.get("/pyroustock/vendas/:empresa_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const vendas = await getByPrefix(`venda_pyrou:${empresaId}:`);
    vendas.sort((a, b) => {
...
legacyRoutes.get("/pyroustock/caixas/:empresa_id", async (c) => {
  try {
    const empresaId = c.req.param("empresa_id");
    const caixas = await getByPrefix(`caixa_pyrou:${empresaId}:`);
    caixas.sort((a, b) => {
  ...
  try {
    ...
    if (!empresa_id || !tipo) {
      return c.json(
        {
          success: false,
          error: "Campos obrigat\xF3rios: empresa_id, tipo"
        },
        400
      );
    }
    ...
    if (tipo === "vendas") {
      const vendas = ids ? await Promise.all(ids.map((id) => get2(`venda_pyrou:${empresa_id}:${id}`))) : await getByPrefix(`venda_pyrou:${empresa_id}:`);
      for (const venda of vendas) {
        if (!venda) continue;
        try {
          const contaId = crypto.randomUUID();
          const dataVencimento = new Date(venda.data);
          await set(`conta_receber:${empresa_id}:${contaId}`, {
            id: contaId,
            empresa_id,
            cliente_id: null,
            ...
            data_recebimento: dataVencimento.toISOString().split("T")[0],
            // Já recebido
            status: "Recebido",
      ...
      const caixas = ids ? await Promise.all(ids.map((id) => get2(`caixa_pyrou:${empresa_id}:${id}`))) : await getByPrefix(`caixa_pyrou:${empresa_id}:`);
      for (const caixa of caixas) {
        if (!caixa) continue;
        try {
          const lancamentoId = crypto.randomUUID();
          const dataFechamento = new Date(caixa.data_fechamento);
          await set(`lancamento:${empresa_id}:${lancamentoId}`, {
            id: lancamentoId,
            empresa_id,
            tipo: "Receita",
            categoria: "Vendas",
            ...
            status: "Efetivado",
            observacoes: `Diferen\xE7a: R$ ${caixa.diferenca?.toFixed(2)}`,
            origem_pyroustock: true,
...
var shouldServeStatic = process.env.SERVE_STATIC === "true";
...
if (shouldServeStatic && existsSync(distDir)) {
  app.get("*", async (c) => {
    ...
    return c.text("Frontend n\xE3o encontrado. Execute npm run build.", 404);
  });
}
serve(
  {
  ...
  () => {
    console.log(`\u{1F680} SisFinance API na porta ${config.port} (0.0.0.0)`);
    if (shouldServeStatic) {
      const distOk = existsSync(join(distDir, "index.html"));
      console.log(`\u{1F4E6} Frontend est\xE1tico: ${distDir} (${distOk ? "ok" : "N\xC3O ENCONTRADO"})`);
    }
    if (!isSupabaseConfigured()) {
      console.warn("\u26A0\uFE0F  SUPABASE_URL n\xE3o configurada");
